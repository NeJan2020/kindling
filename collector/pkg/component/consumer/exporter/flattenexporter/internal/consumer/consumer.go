...
type Metrics interface {
	baseConsumer
	// ConsumeMetrics receives request for consumption.
	ConsumeMetrics(ctx context.Context, request []byte) error
}

// Traces as needed, and sends it to the next processing node if any or to the destination.
type Traces interface {
	baseConsumer
	ConsumeTraces(ctx context.Context, request []byte) error
}
